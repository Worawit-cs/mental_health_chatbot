...
import time as t
import json
import re   # ✅ เพิ่มเพื่อใช้ clean json string
...
# ==================================================================
# Utility: load avatar image bytes
...
def safe_json_parse(resp):
    """รับ JSON ที่อาจห่อด้วย ```json ... ``` แล้วแปลงเป็น dict"""
    if isinstance(resp, dict):
        return resp
    if isinstance(resp, str):
        cleaned = re.sub(r"^```[a-zA-Z]*\s*|\s*```$", "", resp.strip())
        try:
            return json.loads(cleaned)
        except json.JSONDecodeError:
            print("⚠️ JSON parse failed. Returning raw text.")
            return {"advice_answer": cleaned}
    return {"advice_answer": str(resp)}
...
# ✅ FIXED: เพิ่ม parser ที่นี่
def reponse_message(message):
    """เรียก RAG แล้ว parse JSON ให้แน่นอน"""
    raw_resp = RAG().test("USER_INPUT : " + message)["answer"]
    parsed = safe_json_parse(raw_resp)
    print(f"\n=== DEBUGGING TEMP ===\nRAW: {type(raw_resp)}\n{raw_resp}\nPARSED: {parsed}\n=======================\n")
    return parsed
...
def web_page():
    ...
    if st.session_state.count:
        with st.chat_message("assistant", avatar=load_avatar_bytes(BOT_AVATAR_PATH)):
            ans = answer_text_first(TEXT[lang]["first_message"])
            st.write_stream(ans)
            st.session_state.count = False
    else:
        with st.chat_message("assistant", avatar=load_avatar_bytes(BOT_AVATAR_PATH)):
            st.markdown(TEXT[lang]["first_message"])

    # Sidebar
    with st.sidebar:
        st.markdown(TEXT[lang]["language"])
        new_lang = st.selectbox(' ', ['TH','ENG'], index=['TH','ENG'].index(st.session_state.lang))
        if new_lang != st.session_state.lang:
            st.session_state.lang = new_lang
            st.rerun()
        st.write(' ')
        st.write(TEXT[lang]["consult"])
        quick_labels = [item[lang] for item in QUICK_TOPICS]
        ...
        new_key = get_key_from_label(selected_label, lang)
    ...
    # Chat input
    if prompt := st.chat_input(TEXT[lang]["placeholder"]):
        ...
        with st.chat_message("assistant", avatar=load_avatar_bytes(BOT_AVATAR_PATH)):
            with st.spinner("Thinking..."):
                ...
                message = response.get("advice_answer", "I couldn't find reliable context.")
                severity = response.get("severity_level", 0)
                if severity >= 8:
                    popup()
                ans = answer_text(message)
                st.write_stream(ans)
